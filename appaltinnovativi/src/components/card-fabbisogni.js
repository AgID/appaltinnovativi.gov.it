...
const CardFabbisogni = ({ category, image, title, description, publicationDate, proponent, contractor, isOpen }) => {
  return (
        ...
        <CardBody>
          <div className="top-icon mb-2">
            <h6 className="text-primary">
              <img className="icon mr-1" src={LogoFabbisogni} />
              {category}
            </h6>
          </div>
          ...
          <CardReadMore
            ...
            tag="a"
            href={'/fabbisogni/' + title.toLowerCase().replace(/[^a-z0-9 -]/g, '').replace(/\s+/g, '-')}
            text="Leggi di più"
          />
