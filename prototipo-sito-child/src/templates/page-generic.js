...
export default function PageGeneric({ location, data }) {
  ...
  return (
    <Layout location={location}>
      <SEO title={data.strapiPaginaNomeSitoChild.titolo} description="" />
                ...
                <div className="d-flex justify-content-between">
                  <div className="project-intro--logo mw-100">
                    <h1 className="max-w-40 text-600">
                      {data.strapiPaginaNomeSitoChild.titolo}
                    </h1>
                  </div>
                </div>
              </Col>
              ...
              <div className="card card-bg card-big">
                <div className="px-3 px-md-5 card-body pb-0">
                  <div className="card-text">
                    <div className="my-3">
                      {data.strapiPaginaNomeSitoChild.sezione.map((s, index) => {
                        return (
                          <div
                            key={index}
                            className="my-3"
 ...
 //Modificare "strapiPaginaNomeSitoChild" inserendo il nome del sito child in camel case al posto di NomeSitoChild
export const result = graphql`
  query pageQuery($pageId: String) {
    strapiPaginaNomeSitoChild(id: { eq: $pageId }) {
      titolo
      sezione {
        titolo
        descrizione
